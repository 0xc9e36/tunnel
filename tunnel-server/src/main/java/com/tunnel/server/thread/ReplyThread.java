 ...
 * 结果汇报线程
 * 负责如下事务：
 * 接听客户端汇报请求，根据包裹id把汇报结果存起来，供返回给浏览器
 */
public class ReplyThread extends Thread{
    private static final Logger LOGGER = LoggerFactory.getLogger(ReplyThread.class);

    public ReplyThread() {
		this.setName("ReplyThread");
	}
	...
	public void run() {
        ...
        try {  
            ...
            serverSocketChannel.socket().bind(new InetSocketAddress(Config.REPLY_PORT));  
                ...
                while (it.hasNext()) {  
                    ...
                    if(readyKey.isAcceptable()){
                    	...
                    	if(ssc != null){
                    		...
                    		PackManager.setReply(socketChannel);
                    	}
                    }
                }
            }
